...
def test_model(models,tasks,data_loader,count_q,model_key,model_type='mlp',load_bbm=False, ckpt_path='saved_models', robust=False, yield_bias=False, w_neg=1, w_pos=1, debug_regret=False):
    ...
    with torch.no_grad():
        for batch in data_loader:
            inputs, pcReal, prReal, yieldReal = batch
...
def save_models(models,tasks,val_acc,val_regret, best_loss, best_loss_actual,patience, patience_alpha, epoch, counter, continue_training, model_key, val_method='acc', ckpt_path='saved_models'):
