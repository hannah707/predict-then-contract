...
def train_models(models, train_loader,\
                tasks=['rp','yl'],  REG=[0,0], spoplus=False, robust=False, debug_regret=False,\
    ...
    for epoch in range(start_epoch, num_epochs+1):
        ...
        for batch_data in train_loader:
            ...
            if count_q:
                ...
                Q_loss_batch = torch.div(Q_loss_batch,len(inputs))

                if spoplus:
                    Q_loss_plus_batch, _ = Q_loss_spoplus(rpPred, yieldPred, dpReal, rpReal, yieldReal, debug_regret=debug_regret)
                    lossP = torch.add(torch.mul(w11,acc_loss[0]), torch.mul(w12,Q_loss_plus_batch))
                else:
                    lossP = torch.add(torch.mul(w11,acc_loss[0]), torch.mul(w12,Q_loss_batch.clone()))
                
                lossY = torch.add(torch.mul(w21,acc_loss[1]), torch.mul(w22,Q_loss_batch.clone()))
            else:
        ...
        if val_loader is not None:
            val_acc, val_regret = test_model(models,tasks,val_loader,count_q,model_key,robust=robust,yield_bias=yield_bias,w_neg=w_neg,w_pos=w_pos,model_type=model_type, ckpt_path=ckpt_path, debug_regret=debug_regret)
    ...
    if test_loader is not None:
        test_loss_acc, test_regret = test_model(models,tasks,test_loader,count_q=True,load_bbm=True, model_key=model_key,robust=robust, model_type=model_type, ckpt_path=ckpt_path, debug_regret=debug_regret)
    ...
    if save_prog and val_loader is not None:
