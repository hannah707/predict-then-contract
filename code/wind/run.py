...
if __name__ == '__main__':
    # ==================== Parser setting ==========================
    parser = argparse.ArgumentParser(description='Wind Day-Ahead Optimization')
    ...
    parser.add_argument('--hidden_dim1', type=int, default=256, help='neurons in first layer')
    parser.add_argument('--hidden_dim2', type=int, default=512, help='neurons in last layer')
    ...
    parser.add_argument('--data_path', type=str, default='../../inputs/wind/', help='path to input data')
    ...
    parser.add_argument('--use_forecast', type=str2bool, default=False, help='using the \"forecast\" provided by the dataset')
    ...
    daPrice = np.load(os.path.join(args.data_path,'dayahead-prices.npy'))
    features = np.load(os.path.join(args.data_path,'multivariate-features.npy'))
    yl_act = np.load(os.path.join(args.data_path,'generation.npy'))
    # yl_pred = np.load(os.path.join(args.data_path,'forecast.npy')

    features = np.hstack((features, daPrice.reshape(-1,1)))
    assert len(features) == len(daPrice) == len(rtPrice) == len(yl_act), 'length of features mismatch!'
    ...
    if args.take_subset>1:
        ...
        daPrice = daPrice[::args.take_subset]
        features = features[::args.take_subset]
        yl_act = yl_act[::args.take_subset]

    dataset = WindPriceDataset(features, daPrice, rtPrice, yl_act)
    ...
    train_params = {'SEED','num_epochs','lr','val_method','patience','patience_alpha','switch_patience','switch_by_patience','single_warm_up','start_epoch','ckpt_path','save_prog','save_after_switch','exp_key','method','data','spoplus','robust','use_forecast','yield_bias', 'w_neg', 'w_pos'}
