...
constraints_woyl = [decision_var>=0, decision_var<=ac*pc_para+bc]
...
problem_woyl = cp.Problem(objective, constraints_woyl)
assert problem_woyl.is_dpp(), 'The problem is not DPP-compliant.'

clayer = CvxpyLayer(problem, parameters=[pc_para, pr_para, yl_para, pc2_para, pr2_para, pcy_para, pry_para], variables=[decision_var])
clayer_woyl = CvxpyLayer(problem_woyl, parameters=[pc_para, pr_para, yl_para, pc2_para, pr2_para, pcy_para, pry_para], variables=[decision_var])
...
def Q_loss(realtime_price, yield_pred, pcReal, prReal, yieldReal, pc_rmse=0, pr_rmse=0, yl_rmse=yl_rmse, robust=False, spec=False, debug_regret=False):
    ...
    for _pr,_yl,pc,pr,yl in zip(realtime_price, yield_pred, pcReal, prReal, yieldReal):
        ...
        try:
            ...
            se=True
            if debug_regret:
                print("Raise Error:", e, "when using pc, _pr, _yl, pc, pr, yl values:", [i.detach().cpu().numpy() for i in [pc, _pr, _yl, pc, pr, yl]])
...
def Q_loss_woyl(realtime_price, yield_pred, pcReal, prReal, yieldReal, pc_rmse=0, pr_rmse=0, yl_rmse=yl_rmse, robust=False, spec=False, debug_regret=False):
    Q_loss_batch = None
    errored_decision = 0
    Q_losses = []
    ...
    for _pr,_yl,pc,pr,yl in zip(realtime_price, yield_pred, pcReal, prReal, yieldReal):
        ...
        x_opt, = clayer_woyl(pc, pr, yl, pc*pc, pr*pr, pc*yl, pr*yl)
        profit_x_opt = calculate_profit_tensor(x_opt, pc, pr, yl)

        try:
            x_sol, = clayer_woyl(pc, _pr, _yl, pc*pc, _pr*_pr, pc*_yl, _pr*_yl)
            profit_x_sol = calculate_profit_tensor(x_sol, pc, pr, yl)
        except diffcp.cone_program.SolverError as e:
            se=True
            if debug_regret:
